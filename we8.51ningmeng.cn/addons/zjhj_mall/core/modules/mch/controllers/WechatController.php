    ...
    public function actionTemplateMsg()
    {
        if (\Yii::$app->request->isPost) {
            ...
            $tplMsg = [
                ...
                'fxhb' => $fxhbTplMsg,
                'activity' => $activityTplMsg, // 参与活动模板
            ];
            ...
            foreach ($tplMsg as $k => $item) {
                ...
                if (in_array($k, $userAuth) || $k === 'store') {
                    ...
                    if ($k === 'fxhb') {
                        $tplMsg[$k]['plugin'] = '裂变拆红包';
                    }

                    if ($k === 'bargain') {
                        $tplMsg['activity']['plugin'] = '砍价';
                    }
            ...
            // 商城功能用到了模板、该模板就应显示
            if (in_array('bargain', $userAuth)) {
                $tplMsg['activity']['is_show'] = true;
                $name = $tplMsg['activity']['plugin'];
                $tplMsg['activity']['plugin'] = !empty($name) ? $name . '、砍价、发放优惠券' : '砍价、发放优惠券';
            }

            $tplMsg['fxhb']['is_show'] = true;
            $name = $tplMsg['fxhb']['plugin'];
            $tplMsg['fxhb']['plugin'] = !empty($name) ? $name . '、分销佣金' : '分销佣金';

            $tplMsg['mch']['is_show'] = true;
            $name = $tplMsg['mch']['plugin'];
            $tplMsg['mch']['plugin'] = !empty($name) ? $name . '、订单核销' : '订单核销';
